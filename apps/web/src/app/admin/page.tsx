...
export default function AdminDashboard() {
  ...
  // URL dinámica inyectada por GitHub Actions
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">Gestiona los envíos de SkinClear</p>
          </div>
          <div className="bg-white p-2 rounded-xl shadow-sm border border-gray-100 flex items-center gap-2 px-4">
            <Search size={18} className="text-gray-400" />
            <input type="text" placeholder="Buscar cliente..." className="bg-transparent border-none focus:outline-none text-sm w-48 text-black" />
          </div>
        </header>
              ...
              {orders.map((order) => (
                <tr key={order._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="p-4 pl-6">
                    ...
                    <p className="text-[10px] text-gray-400 font-mono mt-0.5">ID: {order._id.substring(0,8)}</p>
                  </td>
                  <td className="p-4 text-center text-sm text-gray-600">{order.city}</td>
                  <td className="p-4 flex justify-center">{getStatusBadge(order.status)}</td>
            ...
            >
              <div className="p-8">
                <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><X size={24} className="text-gray-400" /></button>
                </div>
                ...
                <div className="space-y-8">
                  <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-[10px] font-mono text-gray-400">{selectedOrder.externalReference || 'Sin ref.'}</p>
                    </div>
                  </div>
                  ...
                  <section>
                    ...
                    <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex gap-4">
                      <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm border border-gray-100 flex-shrink-0"><MapPin size={18} className="text-black" /></div>
                      <div>
                  ...
                  <div className="pt-6 border-t border-gray-100 flex justify-between items-center text-[10px] text-gray-400">
                    <div className="flex items-center gap-2"><Hash size={12} /><span>ID: {selectedOrder._id}</span></div>
                    <div className="flex items-center gap-2"><Calendar size={12} /><span>{new Date(selectedOrder.createdAt).toLocaleDateString()}</span></div>
                  </div>
...
function DetailRow({ Icon, label, value, isCopyable = false }: any) {
  return (
    <div className="flex items-center gap-4 group">
      ...
      {isCopyable && (
        <button onClick={() => { navigator.clipboard.writeText(value); alert('Copiado'); }} className="text-xs text-blue-600 font-medium opacity-0 group-hover:opacity-100 transition-opacity">Copiar</button>
      )}
    </div>
  );
}
